...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # 添加1x1卷积分支
        self.conv1x1 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=1)
        self.bn1 = nn.BatchNorm2d(hidden_dim)
        
        # 添加3x3卷积分支
        self.conv3x3 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, padding=1)
        ...
        self.final_conv1x1 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=1)

    def forward(self, input: torch.Tensor):
        # VSS块处理
        x = input + self.drop_path(self.self_attention(self.ln_1(input)))
        
        # 将特征转换为通道优先格式(B, C, H, W)
        B, H, W, C = x.shape
        x_conv = x.permute(0, 3, 1, 2)
        
        # 1x1卷积分支
        branch1 = self.conv1x1(x_conv)
        branch1 = self.bn1(branch1)
        branch1 = self.relu6(branch1)
        
        # 3x3卷积分支
        branch2 = self.conv3x3(x_conv)
        branch2 = self.bn2(branch2)
        branch2 = self.relu6(branch2)
...
class VSSLayer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.is_bottleneck = downsample is None  # 判断是否是瓶颈层

        if self.is_bottleneck:
            # 瓶颈层：使用1个VSS块 + 1个CSCA块
            self.blocks = nn.ModuleList([
                ...
                CSCA_blocks(dim)  # 替换第二个VSS块为CSCA块
            ])
        else:
            # 非瓶颈层：使用原始的VSS块
            self.blocks = nn.ModuleList([
    ...
    def forward(self, x):
        if self.is_bottleneck:
            # 瓶颈层的前向传播
            x = self.blocks[0](x)  # VSS块
            B, H, W, C = x.shape
            x = x.permute(0, 3, 1, 2)  # 转换为CSCA所需的格式 (B, C, H, W)
            x = self.blocks[1](x)  # CSCA块
            x = x.permute(0, 2, 3, 1)  # 转换回原格式 (B, H, W, C)
        else:
            # 非瓶颈层的原始前向传播
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 9, 2], 
        ...
        self.x4_dem_1 = nn.Sequential(
            nn.Linear(dims[3], dims[0]),
            norm_layer(dims[0])
        )
        self.x3_dem_1 = nn.Sequential(
            nn.Linear(dims[2], dims[0]),
            norm_layer(dims[0])
        )
        ...
        # 特征恢复层
        self.x4_3_dem = nn.Sequential(
            nn.Linear(dims[0], dims[2]),
            norm_layer(dims[2])
        )
        self.x3_2_dem = nn.Sequential(
            nn.Linear(dims[0], dims[1]),
            norm_layer(dims[1])
        )
        self.x4_3_2_dem = nn.Sequential(
            nn.Linear(dims[0], dims[1]),
            norm_layer(dims[1])
        )
        self.x2_1_dem = nn.Sequential(
            nn.Linear(dims[0], dims[0]),
            norm_layer(dims[0])
        )
        self.x3_2_1_dem = nn.Sequential(
            nn.Linear(dims[0], dims[0]),
            norm_layer(dims[0])
        )
        self.x4_3_2_1_dem = nn.Sequential(
            nn.Linear(dims[0], dims[0]),
            norm_layer(dims[0])
        )
    ...
    def forward_features(self, x):
        ...
        # 相邻层级特征差异
        def upsample_and_subtract(x_high, x_low):
            B, H, W, C = x_low.shape
            x_high = F.interpolate(x_high.permute(0,3,1,2), size=(H, W), mode='bilinear')
            return abs(x_high.permute(0,2,3,1) - x_low)

        x4_3 = self.x4_x3(upsample_and_subtract(x4_dem_1, x3_dem_1))
        x3_2 = self.x3_x2(upsample_and_subtract(x3_dem_1, x2_dem_1))
        x2_1 = self.x2_x1(upsample_and_subtract(x2_dem_1, x1_dem_1))

        # 跨层级特征差异
        x4_3_2 = self.x4_x3_x2(upsample_and_subtract(x4_3, x3_2))
        x3_2_1 = self.x3_x2_x1(upsample_and_subtract(x3_2, x2_1))
        x4_3_2_1 = self.x4_x3_x2_x1(upsample_and_subtract(x4_3_2, x3_2_1))
        ...
        # 更新跳跃连接
        x_downsample = [x1, x2, x3, x4]
        
        x = self.layers[3](x4)  # 瓶颈层处理
        x = self.norm(x)
