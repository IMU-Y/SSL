...
def main(args):
    ...
    # 有标签数据的dataloader
    train_loader = DataLoader(labeled_trainset, batch_size=args.batch_size, shuffle=True,pin_memory=True, num_workers=0, drop_last=True)
    # 无标签数据的dataloader
    unsupervised_train_loader = DataLoader(unlabeled_trainset, batch_size=args.batch_size, shuffle=True,pin_memory=True, num_workers=0, drop_last=True)

    image_levl = True
    model1, optimizer1 = init_basic_elems(args, image_levl)

    model2, optimizer2 = init_basic_elems(args, image_levl)
    ...
    # 无标签数据，用的是不可靠标签
    unlabeled_trainset = SemiDataset(args.dataset, args.data_root, 'train', args.crop_size, cur_unlabeled_id_path,second=second)
    # 有标签数据的dataloader
    train_loader = DataLoader(labeled_trainset, batch_size=args.batch_size, shuffle=True,pin_memory=True, num_workers=0, drop_last=True)
    # 无标签数据的dataloader
    unsupervised_train_loader = DataLoader(unlabeled_trainset, batch_size=args.batch_size, shuffle=True,pin_memory=True,num_workers=0, drop_last=True)
...
def init_basic_elems(args,image_level=False):
    model_zoo = {'deeplabv3plus': DeepLabV3Plus, 'pspnet': PSPNet, 'deeplabv2': DeepLabV2}
    model = model_zoo[args.model](args.backbone, 4 if args.dataset == 'pascal' else 19,image_level)
...
def train1(model1=None,model2=None, train_loader=None,unsupervised_train_loader=None, valloader=None, criterion=None, optimizer1=None,optimizer2=None, args=None):
    ...
    for epoch in range(args.epochs):
        ...
        for i in tbar:
            ...
            if model1.module.image_level:
                pred_sup_l,_ = model1(img)
                ...
                pred_unsup_l, image_level_pred1 = model1(unsup_imgs)

                pred_sup_r,_ = model2(img)
                ...
                pred_unsup_r, image_level_pred2 = model2(unsup_imgs)
            else:
            ...
            # 计算image-level的损失
            if model1.module.image_level:
                image_level_labels = unsup_minibatch[3].cuda()
                image_level_loss = F.multilabel_soft_margin_loss(image_level_pred1, image_level_labels) + F.multilabel_soft_margin_loss(image_level_pred2, image_level_labels)

                # 总的损失
                loss = loss_sup + loss_sup_r + consistency_weight * (cps_loss_l + cps_loss_un) +  (0.1 - consistency_weight)  * image_level_loss
            else:
                # 总的损失
        ...
        # if MODE == 'train' and ((epoch + 1) in [args.epochs // 5, args.epochs * 4 // 5, args.epochs * 3 // 5, args.epochs * 2 // 5, args.epochs]):
        #     checkpoints.append(deepcopy(model1))
...
def train2(model1=None,model2=None, train_loader=None,unsupervised_train_loader=None, valloader=None, criterion=None, optimizer1=None,optimizer2=None, args=None):
    ...
    for epoch in range(args.epochs):
        ...
        for i in tbar:
            ...
            if model1.module.image_level:
                image_level_labels = unsup_minibatch[3].cuda()
                image_level_loss = F.multilabel_soft_margin_loss(image_level_pred1,
                                                                 image_level_labels) + F.multilabel_soft_margin_loss(
                    image_level_pred2, image_level_labels)
                # 总的损失
                loss = loss_sup + loss_sup_r + consistency_weight * (cps_loss_l + cps_loss_un) +   (0.1 - consistency_weight) * image_level_loss
            else:
                # 总的损失
        ...
        if MODE == 'train' and ((epoch + 1) in [args.epochs // 3, args.epochs * 2 // 3, args.epochs]):
            checkpoints.append(deepcopy(model1))
        # if MODE == 'train' and ((epoch + 1) in [args.epochs // 5, args.epochs * 4 // 5, args.epochs * 3 // 5, args.epochs * 2 // 5,args.epochs]):
        #     checkpoints.append(deepcopy(best_model))
...
if __name__ == '__main__':
    ...
    if args.crop_size is None:
        args.crop_size = {'pascal': 128, 'cityscapes': 721}[args.dataset]
...
# 2022-11-19 修改为bcss 加入imagelevel信息

# 2022-11-24 修改为bcss 加入imagelevel信息 前两个个阶段都加入弱标签
# 目前我们是对可靠伪标签做强增强,之后可以考虑,不可靠也做强增强


# 2022-12-15 弱标注损失
# 0.01 - consistency_weight 
